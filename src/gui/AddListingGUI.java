...
public class AddListingGUI extends JFrame {
	 ...
	 * Create the frame.
	 */
	public AddListingGUI(Database db) {
		...
		JLabel lblNewLabel = new JLabel("Add New Listing");
		...
		btnCreate.setBounds(10, 264, 196, 31);
		...
		btnCreate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				...
				try{
					Database.createListings(tf_Address.getText(), intPrice, Integer.parseInt(sp_RoomCount.getValue().toString()), Integer.parseInt(sp_BathroomCount.getValue().toString()), cb_ListingType.getSelectedItem().toString());
				}catch (Exception e) {
					e.printStackTrace();
				}
				clearWindow( contentPane );
			}
		});
	...
	// Clears all existing text entry fields.
	public void clearWindow(JPanel contentPane ) {
		System.out.println("Clearing Window.");
		tf_Address.setText("");
		tf_Price.setText("");
		sp_RoomCount.setValue(1);
		sp_BathroomCount.setValue(1);
		cb_ListingType.setSelectedIndex(1);
